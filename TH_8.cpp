...
{
	 int m,n;
	 printf("Nhap m la so cot cho ma tran A va B : m="); scanf("%d",&m);
	 printf("Nhap n la so dong cho ma tran A va B : n="); scanf("%d",&n);
	 printf("\n\nNhap cac phan tu cho ma tran A :");
	 for (int i=0;i<m;i++)
	 {
	 	 for (int j=0;j<n;j++)
	 	 {
	 	 	 printf("\n\nA[%d][%d]=",i,j); scanf("%d",A+i*A+j);
	 	 }
	 }
	 for (int i=0;i<m;i++)
	 {
